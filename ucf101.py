...
for epoch in range(start_epoch, start_epoch + 100):
    ...
    if acc > best_acc:
        ...
        if not os.path.isdir('checkpoint'):
            os.mkdir('checkpoint')
        torch.save(state, './checkpoint/ckpt_ucf101_test43-xx1.t7')
        best_acc = acc
        ...
        np.save("best_acc_ucf_cls_test43-xx1.npy", X)
        np.save("best_acc_ucf_clsy_test43-xx1.npy", y)
        X_test, y_test = kNN_ucf101_store(epoch, net, lemniscate, trainloader, testloader, 200, args.nce_t, 0)
        np.save("best_acc_ucf_clst_test43-xx1.npy", X_test)
        np.save("best_acc_ucf_clsyt_test43-xx1.npy", y_test)
        ### modify 0814
